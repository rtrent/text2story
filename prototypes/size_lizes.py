...
def main(input_filename = 'input.txt', output_filetype='.png', bg_color = 'blue', font_name = 'Arial.ttf', font_size=40, font_color='white', alignment='left', shape='post'):
	...
	for line in lines:
		...
		for line in sublines: #breaks long lines into smaller parts
			...
			if y_line_height + font_height > canvas_height:
				...
				image.show()
				image.save(f"output_{file_num}{output_filetype}")
				file_num += 1
			...
			print(f"This line blank: {blank_line}")
			print(f"Last line blank: {blank_line_previous}\n")
			...
			#If the line is not blank, then calculate font alignment and draw text
				...
				draw.text((alignment_x,y_line_height), line.strip(), font = fnt, fill = font_color)
	...
	image.show()
	image.save(f"output{file_num}{output_filetype}")
	sys.exit()
